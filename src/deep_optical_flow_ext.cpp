...
int processflow(const Mat& frame0, const Mat& frame1, Mat& flowx, Mat& flowy, Mat& img)
{
    ...
    Ptr<cv::DenseOpticalFlow> df = cv::optflow::createOptFlow_DeepFlow();
	{
		Mat frame0f;
		Mat frame1f;
		...
		df->calc(frame0f, frame1f, flow);
		const double timeSec = (getTickCount() - start) / getTickFrequency();
		cout << "DeepFlow: " << timeSec << " sec" << endl;
        showFlow("DeepFlow", flow, img);
	}
	Mat planes[2];
...
/*    float alpha,gamma,scale_factor;
    ...
    if (ac > 3) {
        //alpha = float(av[3]);
        std::istringstream iss3( av[3] );
        iss3 >> alpha; }
    else
        alpha = 0.197f;
    if (ac > 4) {
        //gamma = float(av[4]);
        std::istringstream iss4( av[4] );
        iss4 >> gamma; }
    else
        gamma = 50.0f;
    if (ac > 5) {
        //scale_factor = float(av[5]);
        std::istringstream iss5( av[5] );
        iss5 >> scale_factor; }
    else
        scale_factor = 0.8f;
    if (ac > 6) {
        //inner_it = int(av[6]);
        std::istringstream iss6( av[6] );
        iss6 >> inner_it; }
    else
        inner_it = 10;
    if (ac > 7) {
        //outer_it = int(av[7]);
        std::istringstream iss7( av[7] );
        iss7 >> outer_it; }
    else
        outer_it = 77;
    if (ac > 8) {
        //solver_it = int(av[8]);
        std::istringstream iss8( av[8] );
        iss8 >> solver_it; }
    else
        solver_it = 10;

    cout << "Using Brox optic flow parameters:" << endl
         << "   alpha = " << alpha << " = smoothness regularization parameter" << endl
         << "   gamma = " << gamma << " = gradient constancy importance" << endl
         << "   scale_factor = " << scale_factor << " = pyramid scale factor" << endl
