...
count = 0
print 'Comparing true flow to computed Brox flow'
...
#Plot error in flow as function of flow magnitude
plt.clf()
plt.scatter(abs_flow_sample, abs_res_sample)
plt.ylabel('|flow residual|')
plt.xlabel('|flow|')
plt.savefig(img_out + 'flow_abs_res.eps')

plt.clf()
plt.scatter(video_sample, abs_res_sample)
#tck = interpolate.splrep(video_sample,abs_res_sample,s=0)
#xnew = np.arange(0,50,1)
#ynew = interpolate.splev(xnew,tck,der=0)
#plt.plot(xnew, ynew)
plt.ylabel('|flow residual|')
plt.xlabel('intensity')
plt.savefig(img_out + 'intensity_abs_res.eps')
