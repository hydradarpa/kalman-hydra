...
caporig = cv2.VideoCapture("./video/local_prop_cb_with_bud.avi")
cap = cv2.VideoCapture("./video/local_prop_cb_with_bud_densebrox.avi")
#cap = cv2.VideoCapture("./video/20150306_GCaMP_Chl_EC_local_prop.avi")

ret, frame = cap.read()
ret, frame_orig = caporig.read()

filename = './flows/local_prop_cb_with_bud_densebrox_thresh.avi'
fps = 20
framesize = np.shape(frame)[0:2]
fourcc = cv2.VideoWriter_fourcc(*'XVID') 
output = cv2.VideoWriter(filename,fourcc, fps, framesize[::-1])
